...
class MyFrame(EasyFrame):
    ...
    def play_game(self):
        import sys
        sys.path.append("D:\Python Sem 1\GUI Cengage")
        import connneeect
        #game_window.protocol("WM_DELETE_WINDOW", self.on_game_window_close)

    #def on_game_window_close(self):
     #   # Destroy the login page frame when the game window is closed
